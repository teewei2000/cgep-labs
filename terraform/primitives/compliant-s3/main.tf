...
# resource "RESOURCE_TYPE" "LCOAL_NAME"
# tells Terraform what resource am i creating, here it's AWS S3 Bucket
# aws_s3_bucket.primary -> terraform reference (local)
# id = local.primary_name
...
# Create second bucket to receive logs
resource "aws_s3_bucket" "log" {
  bucket = local.log_name
}

# BucketOwnerPreferred means the bucket owner's ownership takes precedence in relevant ACL scenarios.
...
resource "aws_s3_bucket_acl" "log" {
  # Don't create/configure this ACL until the ownership controls have been created.
...
# Enable access logging for the primary bucket and send the logs to the log bucket.
